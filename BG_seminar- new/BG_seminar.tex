...
\end{equation}
where $\vec{\theta}=(\theta_1,\ldots,\theta_{N_l})$ are the variational parameters, unitary $~U_k(\theta_k)= e^{-i\theta_k \hat{O}_k}$ is the unitary generated by the Hermitian operator $\hat{O}_k$, and $W_k$ represents the non-parameterized quantum gates. Typically $\hat{O}_k$ are Pauli strings acting locally on each qubit. In those cases, $U_k$ becomes a product of combinations of single-qubit rotations, each one defined as in Eq.~(\ref{BG:qubit_rotation}). $W_k$ is the entangling unitary constructed from the native gate set. For example for superconducting qubit architectures, CNOT or CZ gates can be used and in trapped ion platforms, XX gates can be used \cite{krantz2019, wright2019}. The choice gate set, qubit topology, and the ordering influence the subspace of the Hilbert space covered by the ansatz and how fast it converges to a solution for a specific problem.
...
\section{Hands-on tutorial: Simulating the hydrogen molecule using VQE}
\label{BG:hands_on}
A large number of executions of quantum circuits is required to evaluate the expectation values.  VQE consists of three nested iterations in order to calculate the energy expectation values:
\begin{enumerate}
