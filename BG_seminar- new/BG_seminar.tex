...
The purpose of this chapter is to give a short introduction to the art of variational quantum algorithms. This chapter is brief but self-contained and only a solid knowledge of quantum mechanics and second quantization is assumed. The reader is encouraged to go through Chapter 4 of Ref.~\cite{nielsen2011} beforehand to better understand the implementation of VQAs on actual quantum devices. 
...
\begin{table}[p]
\centering
\begin{tabular}{m{3cm} m{3cm} m{2cm} m{4.5cm} m{4.5cm} m{3cm} m{1cm}}
...
Transmon qubits & $H_2$ & 2 & hardware-specific parametrized ansatz & PSO & ground and excited states & \cite{colless2018} \\

$Si$ photonic chip & two chlorophyll units in 18-mer ring of LHII complex & 2 & parametrized Hamiltonian
ansatz with truncation scheme & PSO & ground and excited states & \cite{santagati2018} \\
...
\paragraph{Before we begin:}The calculations and simulations shown in this section were done using Qiskit\footnote{At the time of writing, version 0.24.0}. Since $H_2$ is a small molecule, the problem is classically tractable and good classical benchmarking results are already available. Hence the reader will be able to replicate the results on their own with the limited computational resources of a personal computer. Since all the required functions and algorithms are readily available on Qiskit this section will focus more on the theory behind those steps so that the reader can develop an understanding which will enable them to apply the technique other problems on their own. 
