...
%to add definitions and theorems
...
\end{equation}
where $\vec{\theta}=(\theta_1,\ldots,\theta_{N_l})$ are the variational parameters, unitary $~U_k(\theta_k)= e^{-i\theta_k \hat{O}_k}$ is the unitary generated by the Hermitian operator $\hat{O}_k$, and $W_k$ represents the non-parametrized quantum gates. Typically $\hat{O}_k$ are Pauli strings acting locally on each qubit. In those cases, $U_k$ becomes a product of combinations of single-qubit rotations, each one defined as in Eq.~(\ref{BG:qubit_rotation}). $W_k$ is the entangling unitary constructed from the native gate set. For example for superconducting qubit architectures CNOT or CZ gates can be used and in trapped ion platforms XX gates can be used \cite{krantz2019, wright 2019}. The choice gate set, qubit topology and the ordering influences the subspace of the Hilbert space covered by the ansatz and how fast it converges to a solution for a specific problem.
...
\section{Hands-on tutorial: Simulating $\text{H}_2$ molecule using VQE}
\label{BG:hands_on}
With the knowledge of VQAs gained from the previous section we are now well equipped to try out a popular hybrid algorithm VQE to simulate the Hydrogen molecule using Qiskit. Let us start be getting into the specific details of the variational quantum eigensolver.
\begin{itemize}
...
The work presented here is only a part of the continuously growing body of research in variational algorithms\cite{ Quantum Computing: Progress and Prospects. The
