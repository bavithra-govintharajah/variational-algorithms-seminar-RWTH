...
\section{Qiskit hands-on example: Molecular quantum simulation using VQE}
\label{BG:hands_on}
...
\end{enumerate}
Let us now apply the theory learned from previous sections to solve the problem under study in Example 1 from \ref{BG:ground_state_approx}. The goal is to analyze the energy landscape and estimate the ground state energy of $H_2$ molecule using VQE. Fig.~\ref{BG:vqe_loop} summarizes the important steps involved in solving this problem.

\paragraph{Before we begin:}The calculations and simulations given in this section were performed using Qiskit\footnote{At the time of writing, version 0.24.0}. Since $H_2$ is a small molecule, the problem is classically tractable and good classical benchmarking results are already available. Hence the reader will be able to replicate the results on their own with the limited computational resources of a personal computer. Readers interested in the specifics of the classical pre-processing of the electronic structure problem are advised to look up Ref.~\cite{helgaker2000}  and \cite{atkins2010}. 
\begin{figure}[h!]
\centering
...
\label{BG:vqe_loop}
\end{figure}
\subsubsection{Classical pre-processing}
Let us first see how certain choices made during the classical pre-processing will affect the resource requirements for the simulation.
