...
\hline
\end{tabular}
\caption{Representative experimental demonstrations of the VQE algorithm using various quantum computer architectures \cite{cao2019}. \\
...
\end{enumerate}
Let us now apply the theory from previous sections to solve the problem under study in Example 1 from \ref{BG:ground_state_approx}. The goal is to analyze the energy landscape and estimate the ground state energy of $H_2$ molecule using VQE. Fig.~\ref{BG:vqe_loop} summarizes the important steps involved in solving this problem.
...
\begin{dmath}
    H = \underbrace{-\sum_i \frac{\hbar^2}{2m_e}\nabla_i^2 -\sum_\alpha \frac{\hbar^2}{2M_\alpha}\nabla_\alpha^2}_{\text{kinetic energy}}\underbrace{-\sum_{i,\alpha}\frac{e^2}{4\pi \epsilon_0}\frac{Z_\alpha}{\abs{\vec{r}_i - \vec{R}_\alpha}}+\frac{1}{2}\sum_{i\neq j}\frac{e^2}{4\pi \epsilon_0}\frac{1}{\abs{\vec{r}_i - \vec{r}_j}}+\frac{1}{2}\sum_{\alpha\neq \beta}\frac{e^2}{4\pi \epsilon_0}\frac{1}{\abs{\vec{R}_\alpha - \vec{R}_\beta}}}
\end{dmath}
