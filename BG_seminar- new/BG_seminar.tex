...
%
\begin{abstract}
Simulation of large quantum systems or solving large-scale linear algebraic problems is classically inefficient due to high computational costs. Quantum computers promise to unlock these uncharted territories, although fault-tolerant quantum computers will likely not be available soon. Quantum devices available at present have serious bottlenecks such as the limited number of qubits and noise that severely limit circuit depths.
...
To be able to simulate the problem on a personal computer, it's better to choose the basis that will result in fewer qubits. Therefore let's fix the basis set to STO-6G for convenience, but do keep in mind that working in a suitably large basis set is crucial for obtaining accurate results. 
